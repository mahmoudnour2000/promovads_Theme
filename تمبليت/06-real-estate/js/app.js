...
(function () {
  ...
  const { esc, getByCat, getLatest, getTrending, reCard, initCommon } = ReShared;
  const catKeys = ['sale', 'rent', 'investment', 'luxury', 'commercial', 'tips'];
  const sectionColors = {
    sale: '#8b5cf6', rent: '#22c55e', investment: '#f97316',
    luxury: '#eab308', commercial: '#0ea5e9', tips: '#64748b'
  };

  function renderCategorySection(catKey, limit) {
    const cat = RE_CATEGORIES[catKey];
    const articles = getByCat(catKey, limit, true);
    return `<section class="re-section">
      <div class="re-section__head">
        <div class="re-section__info">
          <div class="re-section__icon" style="background:${sectionColors[catKey]}"><i class="fas ${cat.icon}"></i></div>
          <div>
            <h2 class="re-section__name">${esc(cat.name)}</h2>
            <p class="re-section__desc">${esc(cat.desc)}</p>
          </div>
        </div>
        <a href="archive.html?cat=${cat.slug}" class="re-section__btn"><i class="fas fa-arrow-left"></i> ${esc(cat.btnText)}</a>
      </div>
      <div class="grid g3">${articles.map(a => reCard(a)).join('')}</div>
    </section>`;
  }
