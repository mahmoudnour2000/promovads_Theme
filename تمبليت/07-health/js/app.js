...
(function () {
  ...
  const { esc, getByCat, getLatest, getTrending, healthCard, initCommon } = HealthShared;
  const catKeys = ['nutrition', 'fitness', 'mental', 'medicine', 'wellness', 'research'];
  const sectionColors = {
    nutrition: '#22c55e', fitness: '#f97316', mental: '#8b5cf6',
    medicine: '#3b82f6', wellness: '#14b8a6', research: '#6366f1'
  };

  function renderCategorySection(catKey, limit) {
    const cat = HEALTH_CATEGORIES[catKey];
    const articles = getByCat(catKey, limit, true);
    return `<section class="health-section">
      <div class="health-section__head">
        <div class="health-section__info">
          <div class="health-section__icon" style="background:${sectionColors[catKey]}"><i class="fas ${cat.icon}"></i></div>
          <div>
            <h2 class="health-section__name">${esc(cat.name)}</h2>
            <p class="health-section__desc">${esc(cat.desc)}</p>
          </div>
        </div>
        <a href="archive.html?cat=${cat.slug}" class="health-section__btn"><i class="fas fa-arrow-left"></i> ${esc(cat.btnText)}</a>
      </div>
      <div class="grid g3">${articles.map(a => healthCard(a)).join('')}</div>
    </section>`;
  }
