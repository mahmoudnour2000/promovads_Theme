/* صحة+ — Single article page */
(function () {
  'use strict';

  const { esc, getArticle, getByCat, healthCard, initCommon } = HealthShared;
  ...
  function buildBody(a) {
    ...
    return `
      <p>${esc(a.excerpt)}</p>
      <p>في تغطية <strong>صحة+</strong> المتخصصة، نقدّم معلومات موثوقة حول <strong>${esc(cat.name)}</strong> — مدعومة بمصادر علمية ومراجع طبية. ${esc(a.author)} يشرح التفاصيل والتوصيات العملية للقراء.</p>
      <div class="info-box" style="background:#f0fdfa;border-right:4px solid #14b8a6;border-radius:8px 0 0 8px;padding:16px 20px;margin:24px 0">
      ...
      <blockquote style="background:#f8fafc;border-right:4px solid #14b8a6;padding:16px 20px;border-radius:8px 0 0 8px;color:#334155">"${esc(a.title)} — معلومة مهمة لكل من يهتم بـ ${esc(cat.name)}." — ${esc(a.author)}</blockquote>
      <h2>الخلاصة</h2>
      <p>تابع صحة+ للحصول على آخر الأخبار والإرشادات الصحية. ${esc(a.author)} يعد بتغطية مستمرة لكل ما يهم في ${esc(cat.name)}.</p>`;
  }
  ...
  function renderArticle() {
    ...
    document.title = `${a.title} — صحة+`;
    const prev = HEALTH_ARTICLES.find(x => x.id === a.id - 1);
    const next = HEALTH_ARTICLES.find(x => x.id === a.id + 1);
    ...
    el.innerHTML = `
      ...
      <div class="article-meta">
        <div class="author">
          ...
          <div><strong>${esc(a.author)}</strong><div style="font-size:.72rem;color:#14b8a6">كاتب — ${esc(cat.name)}</div></div>
        </div>
      ...
      <figure class="article-feat">
        ...
        <figcaption>${esc(a.title)} — صحة+</figcaption>
      </figure>
      <div class="entry-content">${buildBody(a)}</div>
      <div class="article-tags"><h4>الوسوم</h4><div class="tags">${HEALTH_TAGS.slice(0, 4).map(t => `<a href="#" class="tag">${esc(t)}</a>`).join('')}</div></div>
      <div class="author-box">
        <img src="https://i.pravatar.cc/120?img=${a.avatar}" alt="" class="author-box__avatar">
        <div><h3 class="author-box__name">${esc(a.author)}</h3><div class="author-box__role">كاتب صحي — صحة+</div>
      ...
      <section class="related">
        ...
        <div class="grid g3" style="margin-top:20px">${related.map(r => healthCard(r)).join('')}</div>
      </section>`;
