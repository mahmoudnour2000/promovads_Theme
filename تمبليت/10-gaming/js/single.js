/* جيمر+ — Single article page */
(function () {
  'use strict';

  const { esc, getArticle, getByCat, gameCard, initCommon } = GameShared;
  ...
  function buildBody(a) {
    ...
    return `
      <p>${esc(a.excerpt)}</p>
      <p>في تغطية <strong>جيمر+</strong>، نتابع كل تفاصيل <strong>${esc(cat.name)}</strong> — من الإطلاقات إلى بطولات eSports. ${esc(a.author)} يقدّم تحليلاً شاملاً للحدث وتداعياته على مجتمع اللاعبين.</p>
      <div class="info-box" style="background:#faf5ff;border-right:4px solid #a855f7;border-radius:8px 0 0 8px;padding:16px 20px;margin:24px 0">
        <strong style="color:#9333ea;display:block;margin-bottom:4px"><i class="fas fa-info-circle"></i> ملخص سريع</strong>
        <p style="color:#64748b;margin:0;font-size:.88rem">${esc(a.excerpt)}</p>
      </div>
      <h2>تفاصيل الحدث</h2>
      <p>التقييمات وردود الجمهور تؤكد أن ${esc(a.title)} من أبرز أحداث ${esc(cat.name)} هذا الموسم. اللاعبون والمراجعون يتفاعلون بقوة مع ما تم الإعلان عنه.</p>
      <ul>
        <li>تحليل الميزات الجديدة والتغييرات</li>
        <li>تأثير الحدث على السوق والمجتمع</li>
        <li>ردود فعل المطورين واللاعبين المحترفين</li>
        <li>ما التالي في ${esc(cat.name)}؟</li>
      </ul>
      <blockquote style="background:#f8fafc;border-right:4px solid #a855f7;padding:16px 20px;border-radius:8px 0 0 8px;color:#334155">"${esc(a.title)} — لحظة لن ينساها عشاق ${esc(cat.name)}." — ${esc(a.author)}</blockquote>
      <h2>الخلاصة</h2>
      <p>تابع جيمر+ للحصول على آخر التحديثات والمراجعات. ${esc(a.author)} يعد بتغطية مستمرة لكل ما يحدث في ${esc(cat.name)}.</p>`;
  }
  ...
  function renderArticle() {
    ...
    document.title = `${a.title} — جيمر+`;
    const prev = GAME_ARTICLES.find(x => x.id === a.id - 1);
    const next = GAME_ARTICLES.find(x => x.id === a.id + 1);
    ...
    el.innerHTML = `
      ...
      <div class="article-meta">
        <div class="author">
          ...
          <div><strong>${esc(a.author)}</strong><div style="font-size:.72rem;color:#a855f7">مراسل — ${esc(cat.name)}</div></div>
        </div>
      ...
      <figure class="article-feat">
        ...
        <figcaption>${esc(a.title)} — جيمر+</figcaption>
      </figure>
      <div class="entry-content">${buildBody(a)}</div>
      <div class="article-tags"><h4>الوسوم</h4><div class="tags">${GAME_TAGS.slice(0, 4).map(t => `<a href="#" class="tag">${esc(t)}</a>`).join('')}</div></div>
      <div class="author-box">
        <img src="https://i.pravatar.cc/120?img=${a.avatar}" alt="" class="author-box__avatar">
        <div><h3 class="author-box__name">${esc(a.author)}</h3><div class="author-box__role">مراسل ألعاب — جيمر+</div>
        <p class="author-box__bio">يغطي ${esc(a.author)} ${esc(cat.name)} منذ سنوات — مراجعات وتقارير من قلب المجتمع.</p></div>
      </div>
      <nav class="post-nav">
      ...
      <section class="related">
        ...
        <div class="grid g3" style="margin-top:20px">${related.map(r => gameCard(r)).join('')}</div>
      </section>`;
