...
(function () {
  ...
  const { esc, getByCat, getLatest, getTrending, gameCard, initCommon } = GameShared;
  const catKeys = ['pc', 'console', 'mobile', 'esports', 'reviews', 'releases'];
  const sectionColors = {
    pc: '#a855f7', console: '#6366f1', mobile: '#22c55e',
    esports: '#f97316', reviews: '#ec4899', releases: '#0ea5e9'
  };
    ...
    return `<section class="game-section">
      <div class="game-section__head">
        <div class="game-section__info">
          <div class="game-section__icon" style="background:${sectionColors[catKey]}"><i class="fas ${cat.icon}"></i></div>
          <div>
            <h2 class="game-section__name">${esc(cat.name)}</h2>
            <p class="game-section__desc">${esc(cat.desc)}</p>
          </div>
        </div>
        <a href="archive.html?cat=${cat.slug}" class="game-section__btn"><i class="fas fa-arrow-left"></i> ${esc(cat.btnText)}</a>
      </div>
      <div class="grid g3">${articles.map(a => gameCard(a)).join('')}</div>
    </section>`;
  }
