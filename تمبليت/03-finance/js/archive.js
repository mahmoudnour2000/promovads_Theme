...
(function () {
  ...
  const { esc, getByCat, getTrending, articleCard, initCommon } = FinanceShared;
  ...
  function renderArchiveHeader(catKey) {
    ...
    document.title = `${cat.name} — أرشيف مال+`;
  }
  ...
  function renderGrid(catKey) {
    ...
    el.innerHTML = getByCat(catKey).map(a => articleCard(a, catKey === 'analysis')).join('');
  }
  ...
  function renderSidebar(catKey) {
    ...
    el.innerHTML = `
      <div class="widget"><div class="widget__title">الأقسام</div><div class="widget__body" style="padding:8px 12px">
        ${catKeys.map(k => {
          const c = FIN_CATEGORIES[k];
          const n = getByCat(k).length;
          const active = k === catKey ? 'color:#15803d;font-weight:800' : 'color:var(--text-muted)';
          return `<a href="archive.html?cat=${k}" style="display:flex;justify-content:space-between;padding:10px 6px;border-bottom:1px dashed var(--fin-border);font-size:.84rem;${active}"><span>${esc(c.name)}</span><span style="background:${c.color};color:#fff;padding:2px 8px;border-radius:999px;font-size:.68rem;font-weight:700">${n}</span></a>`;
        }).join('')}
      </div></div>
      <div class="widget"><div class="widget__title">🔥 الأكثر قراءة</div><div class="widget__body" style="padding:8px 16px">
        ${trending.map((a, i) => `<div class="trending-item" style="display:flex;gap:10px;padding:8px 0;border-bottom:1px dashed var(--fin-border)"><span class="n">${String(i + 1).padStart(2, '0')}</span><div><h4 style="font-size:.82rem;margin-bottom:3px"><a href="single.html?id=${a.id}">${esc(a.title)}</a></h4><span style="font-size:.72rem;color:var(--text-muted)">${esc(a.views)} · ${esc(a.time)}</span></div></div>`).join('')}
      </div></div>`;
  }

  function setActiveNav(catKey) {
    document.querySelectorAll('.fin-nav__item').forEach(el => el.classList.remove('active'));
    const item = document.querySelector(`.fin-nav__item[data-cat="${catKey}"]`);
    if (item) item.classList.add('active');
  }

  document.addEventListener('DOMContentLoaded', () => {
    const catKey = catKeys.includes(getParam('cat')) ? getParam('cat') : 'mk';
