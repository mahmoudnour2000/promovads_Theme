/* سبورت+ — Single article page */
(function () {
  'use strict';

  const { esc, getArticle, getByCat, sportCard, initCommon } = SportShared;
  ...
  function buildBody(a) {
    ...
    return `
      <p>${esc(a.excerpt)}</p>
      <p>في تقرير من <strong>سبورت+</strong>، نتابع كل تفاصيل <strong>${esc(cat.name)}</strong> — من الملعب إلى غرفة الأخبار. ${esc(a.author)} يقدّم تحليلاً شاملاً للحدث وتداعياته على البطولات والفرق.</p>
      <div class="info-box" style="background:#fff5f5;border-right:4px solid #e63329;border-radius:8px 0 0 8px;padding:16px 20px;margin:24px 0">
        <strong style="color:#c02820;display:block;margin-bottom:4px"><i class="fas fa-info-circle"></i> ملخص الخبر</strong>
        <p style="color:#64748b;margin:0;font-size:.88rem">${esc(a.excerpt)}</p>
      </div>
      <h2>تفاصيل الحدث</h2>
      <p>الأرقام والإحصائيات تؤكد أن ${esc(a.title)} من أبرز أحداث ${esc(cat.name)} هذا الموسم. الجماهير والمحللون يتفاعلون بقوة مع ما حدث على أرض الملعب.</p>
      <ul>
        <li>تحليل الأداء الفردي والجماعي للفرق</li>
        <li>تأثير النتيجة على ترتيب البطولة</li>
        <li>ردود فعل اللاعبين والمدربين</li>
        <li>ما التالي في جدول المباريات؟</li>
      </ul>
      <blockquote style="background:#f8fafc;border-right:4px solid #e63329;padding:16px 20px;border-radius:8px 0 0 8px;color:#334155">"${esc(a.title)} — لحظة لن ينساها عشاق ${esc(cat.name)}." — ${esc(a.author)}</blockquote>
      <h2>الخلاصة</h2>
      <p>تابع سبورت+ للحصول على آخر الأخبار والتحليلات. ${esc(a.author)} يعد بتغطية مستمرة لكل ما يحدث في ${esc(cat.name)}.</p>`;
  }
  ...
  function renderArticle() {
    ...
    document.title = `${a.title} — سبورت+`;
    const prev = SPORT_ARTICLES.find(x => x.id === a.id - 1);
    const next = SPORT_ARTICLES.find(x => x.id === a.id + 1);
    ...
    el.innerHTML = `
      ...
      <div class="article-meta">
        <div class="author">
          ...
          <div><strong>${esc(a.author)}</strong><div style="font-size:.72rem;color:#e63329">مراسل — ${esc(cat.name)}</div></div>
        </div>
      ...
      <figure class="article-feat">
        ...
        <figcaption>${esc(a.title)} — سبورت+</figcaption>
      </figure>
      <div class="entry-content">${buildBody(a)}</div>
      <div class="article-tags"><h4>الوسوم</h4><div class="tags">${SPORT_TAGS.slice(0, 4).map(t => `<a href="#" class="tag">${esc(t)}</a>`).join('')}</div></div>
      <div class="author-box">
        <img src="https://i.pravatar.cc/120?img=${a.avatar}" alt="" class="author-box__avatar">
        <div><h3 class="author-box__name">${esc(a.author)}</h3><div class="author-box__role">مراسل رياضي — سبورت+</div>
        <p class="author-box__bio">يغطي ${esc(a.author)} ${esc(cat.name)} منذ سنوات — تقارير وتحليلات رياضية من الملعب.</p></div>
      </div>
      <nav class="post-nav">
      ...
      <section class="related">
        ...
        <div class="grid g3" style="margin-top:20px">${related.map(r => sportCard(r)).join('')}</div>
      </section>`;
