/* كريبتو+ — Single article page */
(function () {
  'use strict';

  const { esc, getArticle, getByCat, cryptoCard, initCommon } = CryptoShared;
  ...
  function buildBody(a) {
    ...
    return `
      <p>${esc(a.excerpt)}</p>
      <p>في تغطية <strong>كريبتو+</strong>، نتابع كل تطورات <strong>${esc(cat.name)}</strong> — من on-chain data إلى قرارات التنظيم. ${esc(a.author)} يقدّم تحليلاً شاملاً للحدث وتداعياته على السوق.</p>
      <div class="info-box" style="background:#fff8f0;border-right:4px solid #f7931a;border-radius:8px 0 0 8px;padding:16px 20px;margin:24px 0">
        <strong style="color:#e07d0e;display:block;margin-bottom:4px"><i class="fas fa-info-circle"></i> ملخص سريع</strong>
        <p style="color:#64748b;margin:0;font-size:.88rem">${esc(a.excerpt)}</p>
      </div>
      <h2>تفاصيل الحدث</h2>
      <p>البيانات on-chain والأسعار تؤكد أن ${esc(a.title)} من أبرز أحداث ${esc(cat.name)} هذا الأسبوع. المستثمرون والمحللون يتفاعلون بقوة مع ما يحدث في السوق.</p>
      <ul>
        <li>تحليل تأثير الحدث على سعر BTC والعملات البديلة</li>
        <li>ردود فعل البورصات والمؤسسات</li>
        <li>توقعات المحللين للأسبوع القادم</li>
        <li>ما الذي يجب متابعته في ${esc(cat.name)}؟</li>
      </ul>
      <blockquote style="background:#f8fafc;border-right:4px solid #f7931a;padding:16px 20px;border-radius:8px 0 0 8px;color:#334155">"${esc(a.title)} — لحظة محورية لعشاق ${esc(cat.name)}." — ${esc(a.author)}</blockquote>
      <h2>الخلاصة</h2>
      <p>تابع كريبتو+ للحصول على آخر التحديثات وأسعار العملات. ${esc(a.author)} يعد بتغطية مستمرة لكل ما يحدث في ${esc(cat.name)}.</p>`;
  }
  ...
  function renderArticle() {
    ...
    document.title = `${a.title} — كريبتو+`;
    const prev = CRYPTO_ARTICLES.find(x => x.id === a.id - 1);
    const next = CRYPTO_ARTICLES.find(x => x.id === a.id + 1);
    ...
    el.innerHTML = `
      ...
      <div class="article-meta">
        <div class="author">
          ...
          <div><strong>${esc(a.author)}</strong><div style="font-size:.72rem;color:#f7931a">محلل — ${esc(cat.name)}</div></div>
        </div>
      ...
      <figure class="article-feat">
        ...
        <figcaption>${esc(a.title)} — كريبتو+</figcaption>
      </figure>
      <div class="entry-content">${buildBody(a)}</div>
      <div class="article-tags"><h4>الوسوم</h4><div class="tags">${CRYPTO_TAGS.slice(0, 4).map(t => `<a href="#" class="tag">${esc(t)}</a>`).join('')}</div></div>
      <div class="author-box">
        <img src="https://i.pravatar.cc/120?img=${a.avatar}" alt="" class="author-box__avatar">
        <div><h3 class="author-box__name">${esc(a.author)}</h3><div class="author-box__role">محلل كريبتو — كريبتو+</div>
        <p class="author-box__bio">يغطي ${esc(a.author)} ${esc(cat.name)} منذ سنوات — تحليلات on-chain وتقارير سوقية.</p></div>
      </div>
      <nav class="post-nav">
      ...
      <section class="related">
        ...
        <div class="grid g3" style="margin-top:20px">${related.map(r => cryptoCard(r)).join('')}</div>
      </section>`;
