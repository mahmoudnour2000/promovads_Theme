...
(function () {
  ...
  const { esc, getByCat, getLatest, getTrending, cryptoCard, initCommon } = CryptoShared;
  const catKeys = ['bitcoin', 'altcoins', 'defi', 'nft', 'regulation', 'analysis'];
  const sectionColors = {
    bitcoin: '#f7931a', altcoins: '#6366f1', defi: '#22c55e',
    nft: '#ec4899', regulation: '#0ea5e9', analysis: '#f97316'
  };

  function renderCategorySection(catKey, limit) {
    const cat = CRYPTO_CATEGORIES[catKey];
    const articles = getByCat(catKey, limit, true);
    return `<section class="crypto-section">
      <div class="crypto-section__head">
        <div class="crypto-section__info">
          <div class="crypto-section__icon" style="background:${sectionColors[catKey]}"><i class="fas ${cat.icon}"></i></div>
          <div>
            <h2 class="crypto-section__name">${esc(cat.name)}</h2>
            <p class="crypto-section__desc">${esc(cat.desc)}</p>
          </div>
        </div>
        <a href="archive.html?cat=${cat.slug}" class="crypto-section__btn"><i class="fas fa-arrow-left"></i> ${esc(cat.btnText)}</a>
      </div>
      <div class="grid g3">${articles.map(a => cryptoCard(a)).join('')}</div>
    </section>`;
  }
